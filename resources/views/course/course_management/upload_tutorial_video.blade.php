...
@section('manage_course')
    <div class="card card_width_center">
        ...
        <div class="card-body">
            ...
            <form method="post" action="{{route('course.create_tutorial_video',['course_id'=>$course->id,'lesson_id'=>$lesson->id])}}" enctype="multipart/form-data">

                @csrf

                <div class="form-group">
                    <label for="title">{{__('text.title')}}</label>
                    <input type="text" class="form-control {{ $errors->has('title') ? ' is-invalid' : '' }}"  id="title" name="title" placeholder="{{__('text.Tutorial Video Title')}}" value="{{old('title')}}">
                    @if ($errors->has('title'))
                ...
                <div class="form-group">
                    ...
                    <div class="mt-2" id="show_tutorial_video"></div>
                </div>
